...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Wave{
        constructor(lw, rw){
            this.leftwave = lw
            this.rightwave = rw
            let xvalues = [lw.x, lw.cx, lw.ex, rw.x, rw.cx, rw.ex]
            let yvalues = [lw.y, lw.cy, lw.ey, rw.y, rw.cy, rw.ey]
            this.box = new Rectangle(Math.min(...xvalues),Math.min(...yvalues),(Math.max(...xvalues)-Math.min(...xvalues)),(Math.max(...yvalues)-Math.min(...yvalues)), "#ff000020")
        }
        draw(){
            this.box.draw()
            this.leftwave.draw()
    ...
    let water = new Rectangle(0,450, 1280, 400, "#00aaFF33")
    let trig = new Triangle(350, 350, "green", 100)

    let wave = new Wave(new Bezzy(400, 450, 300, 300, 300, 450), new Bezzy(250, 350, 350, 400, 100, 450))
